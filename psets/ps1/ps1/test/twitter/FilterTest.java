...
public class FilterTest {
     ...
     * Testing Strategy..
     * 
     * tweets-size: 0, 1, many 
     * tweets: haveOnlyUsername, mixOfUsers username:
     * presentInTweets, notPresentInAnyTweet
     */
    ...
    private static final Tweet tweet3 = new Tweet(1, "alyssa", "is it reasonable to talk about rivest so much?", d1);
    private static final Tweet tweet4 = new Tweet(1, "alyssa", "snaskerit kdsjfldskjflksj ksdjflksdjflksdj?", d1);
    ...
    // covers many tweet list, usernameNotPresent
    @Test
    public void testWrittenByMultipleTweetsNoResult() {
        List<Tweet> writtenBy = Filter.writtenBy(Arrays.asList(tweet1, tweet2, tweet3), "akhil");

        assertTrue("expected list to contain tweet", writtenBy.isEmpty());
    }

    // covers tweet-size many, username multiple-tweets, mixOfUsers
    @Test
     ...
     * 
     * tweets-size: 0, 1, many 
     * timespan: everyTweetInside, NoTweetInsideIt, someInSomeOut
     */

    
    // covers NoTweets, AllTweetInside
    @Test
    public void testInTimespanAllTweetsNoResults() {
    ...
    // covers ManyTweets, someInsomeOut
    @Test
    ...
    // covers ManyTweets, NoTweetInside
    @Test
     ...
     * 
     * tweets-size: 0,1,2 
     * words:       someWordinTweet,NotAnyWordInAnyTweet,
     */
    
    // covers someTweetsOK other not
    @Test
    public void testContainingMultipleTweets() {
        List<Tweet> containing = Filter.containing(Arrays.asList(tweet1, tweet2, tweet4), Arrays.asList("talk"));

        assertFalse("expected non-empty list", containing.isEmpty());
        assertEquals("expected size is 2", 2, containing.size());
